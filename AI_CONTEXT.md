...
| 2026-08-15 | `<pending>` | `fix(ux): clarify restart and back navigation` | One authoritative full reset (`handleRestartAssessment` → Assessment Mode); Re-take MMSE = MMSE-only reset; Back = navigation without clearing the session; header has explicit "Re-take MMSE" + "Restart Assessment"; new Backs from MMSE intro and Analysis |
