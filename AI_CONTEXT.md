...
- **Request body (batch):**
  ...
  - **Deterministic (no AI):** `orientation_time.*` (year / season / date / day / month) is scored 100% deterministically in `backend/src/mmse_rules.py` against the server's current date/time. These five items NEVER reach the AI provider: `DETERMINISTIC_SECTIONS` in `ai_eval.py` splits them out of the batch before any provider call, so the model payload contains only AI-required items. Results always carry `confidence: 1.0` (never flagged for review) and an unparseable response scores `correct: false` — it never becomes a per-item error, so the Summary is never stuck.
...
### Current State
- Working tree: Orientation-to-Time deterministic-scoring fix in progress (uncommitted; `backend/src/ai_eval.py` modified, `backend/src/mmse_rules.py` + `backend/tests/test_mmse_rules.py` new). Git branch `main`, tracking `origin/main`. All prior milestones pushed. `npm run build` passes; backend tests pass (46/46); `/predict` verified HTTP 200 with the unchanged 5-field payload; `/mmse/evaluate` verified live (5/5 deterministic orientation items + AI batch, 0 errors). Ollama 0.32.9 installed with `gemma3:4b` aliased as `gemma3:latest`; Gemma 3 4B supports vision.
