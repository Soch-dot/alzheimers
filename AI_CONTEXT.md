...
- **Structure:** 11 sections, max total **30**:
  ...
  6. Naming (2) — wristwatch, pencil; **hybrid** (wristwatch≡watch, articles stripped; ambiguous descriptions → AI).
...
### Current State
- Working tree: expanded deterministic MMSE scoring fix in progress (uncommitted; `backend/src/ai_eval.py`, `backend/src/mmse_rules.py`, `backend/tests/test_mmse_rules.py`, `AI_CONTEXT.md` modified). Git branch `main`, tracking `origin/main`. All prior milestones pushed. `npm run build` passes; backend tests pass (85/85); `/predict` verified HTTP 200 with the unchanged 5-field payload; live hybrid MMSE verified (clean 30-item benchmark: deterministic=29, ambiguous=0, ai=1, one provider call, 21.66s, 30/30, 0 errors; wrong answers deterministic; ambiguous items routed to AI). Ollama 0.32.9 installed with `gemma3:4b` aliased as `gemma3:latest`; Gemma 3 4B supports vision.
...
- `backend/src/mmse_rules.py` (hybrid engine: `SPELL_WORLD_EXPECTED`, sentinels `AMBIGUOUS`/`MATCH`/`NO_MATCH`, `_object_verdict`/`_repetition_verdict`/`_item_position`/`_result`, `_normalize_text`/`_strip_articles`/`_is_uncertain`/`_is_generic_or_referential`, `_EQUIVALENTS`, new evaluators `evaluate_attention_spell_world`/`evaluate_orientation_place`/`evaluate_registration`/`evaluate_delayed_recall`/`evaluate_naming`/`evaluate_repetition`, `evaluate_attention_serial7`/`evaluate_orientation_time` extended with `expected`)
