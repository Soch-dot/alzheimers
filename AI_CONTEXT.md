...
## 0. Production Rollback Record (authorized)

- **Rollback performed:** 2026-08-16. Production restored to the 3-class Random Forest pipeline.
- **Last known-good base:** commit `6b7d39f` (pre-ML-integration production state).
- **Restored production:** `backend/models/best_model.pkl` (RandomForest, 3 classes: 0=Nondemented, 1=Converted, 2=Demented); original `/predict` 3-class contract (`alzheimers_detected`, `detection_percentage`, `predicted_class`, `class_index`, `probabilities`, `rule_applied`, `rule_usage_percentage`); original frontend and `backend/models/shap/` (TreeExplainer, RF).
- **Research preserved on branch:** `experiment/binary-lr-calibration` (binary LR, calibration experiments, threshold, longitudinal, feature-feasibility, probability audit, and the production-integration history commits `8eac181`, `500405f`, `4299755`). Do NOT merge this branch into main.
- **No LR/calibrator production behavior remains active.** The sigmoid calibrator (`sigmoid_calibrator_v1.pkl`), binary threshold (0.40), and binary screening fields are NOT part of restored production.
- **Binary/LR work is research-only unless separately authorized later.**
...
- **Mic control + Enter navigation + Patient-mode explainability (UX-fix milestone):**
  ...
  - **Patient-mode Next explainability:** original root cause of the "all 5 place responses but Next disabled" issue — `sectionResponseCounts().orientationPlace` requires BOTH a configured location field AND a response, and pure Patient mode never sets the location (examiner form hidden), so `done` stayed 0 and completion logic blocked Next. **SUPERSEDED** by the pending-examiner milestone below: Patient mode can now CONTINUE past Orientation to Place (and Q11) once its input is complete, with the section marked "Pending examiner verification" instead of being hard-blocked.
- **Patient flow with pending examiner sections (UX-fix milestone):** pure Patient mode no longer gets permanently stuck at examiner-dependent sections. No score is ever fabricated and nothing is marked correct/incorrect without the examiner.
  - **Explicit `SectionStatus` model (`frontend/src/mmse/batch.ts`):** `'incomplete' | 'pending-examiner' | 'complete'` via `getSectionStatus(id, state, mode)` + `canNavigateSection(id, state, mode, phase)`. Patient-mode navigation: response incomplete → Next/Enter disabled; response complete but not scoreable without the examiner → Next/Enter ENABLED + "Pending examiner verification" (no score); scoreable → normal. Examiner / Examiner + Patient modes are unchanged (strict: a configured location / finalized Copying is required).
  - **Mode-aware counts:** `sectionResponseCounts(state, mode?)` and `isSectionResponseComplete(id, state, mode?)` now count the PATIENT's input for Orientation to Place (all 5 responses) and treat Copying as the patient portion done in Patient mode; examiner semantics are preserved for examiner/combined modes.
  - **Orientation to Place (patient):** all 5 responses → Next enabled, Enter advances to Section 3, the in-section box and nav message say "Pending examiner verification" (never "Examiner required"), responses are stored with NO `aiScore`/`manual` (status `idle`) — no deterministic credit for merely answering. Missing responses still block navigation.
  - **Q11 Copying (patient):** shows "Pending examiner verification", Next/Enter work, no photo is required, no Analyze action, and `copying.status` stays `'empty'` with no `aiScore`/`manual` — no fake score. The patient can finish the patient-response portion.
  - **Final summary gating (patient):** if any section is pending-examiner, the summary shows "Assessment requires examiner verification" with a list ("Orientation to Place — Pending examiner verification", "Copying — Pending examiner verification"); it does NOT show "MMSE Assessment Complete", does NOT offer "Continue to Analysis", and the score is explicitly non-final. The pending card survives a refresh.
  - **Hand off to examiner:** a "Hand off to examiner" control (on the pending summary card and in the Section 2/Q11 pending boxes) calls `handleHandoffToExaminer` (`App.tsx`) — switches the session mode to `'examiner'` WITHOUT losing any data (mode, details, responses, step all preserved, persisted immediately). The pending card is replaced by the normal collect summary; the examiner can then configure the location, run the AI batch, and take/upload the Q11 photo to finalize the score.
  - **Examiner + Patient unchanged:** with the location configured, Orientation to Place is scored normally, no pending state appears, and the full flow (incl. Q11 photo analysis) finalizes the score.
  - Pending state is DERIVED from the persisted MMSE state (responses + location + copying), so the session schema and `session.ts` are unchanged — refresh/persistence still works. **Verified in headless Chrome (CDP, no new deps) — 40/40 checks PASS** (Patient flow, Enter, refresh, Q11, summary gating, hand-off, Examiner + Patient scoring); the previous session-persistence suite re-ran 41/41 PASS (no regression).
- **Clarified restart / re-take / back navigation (UX-fix milestone):** the three actions are now unambiguously separated. **Restart Assessment = full reset** (clears mode, details, MMSE responses/AI scores/review state, Q6/Q11 state, prediction, and the persisted `alzheimers_assessment_session_v1` key + legacy key via `clearSession()`, resets all app state, and returns to Assessment Mode — the FIRST step). **Re-take MMSE = MMSE-only reset** (preserves mode + Age/Sex/Education/SES, clears MMSE responses/AI results/score/Q6/Q11 state, returns to the MMSE intro — never to Assessment Mode). **Back = pure navigation** (previous logical step, NEVER clears the session).
  - **One authoritative full reset (`App.tsx`):** `handleRestartAssessment()` (formerly `handleRestart`) is THE single function reused by every "Restart Assessment" control — Assessment Details, the MMSE summary footer, the Analysis form, and a NEW "Restart Assessment" button in the MMSE section header. It calls `clearSession()` (removes `alzheimers_assessment_session_v1` AND the legacy `alzheimers_assessment_details_v1` key), resets `mode`/`details`/`mmse`/`mmseScore`/`result`/`error`, and sets `phase='mode'`. The empty-session guard means the cleared key is never re-written → a refresh after restart still lands on Assessment Mode.
  - **MMSE header controls made explicit:** the header's old "Restart" button (which actually re-took the MMSE) is now TWO explicit buttons — "Re-take MMSE" (confirm + local MMSE reset + calls App `handleRetake`, which flushes the cleared MMSE part to the session immediately) and "Restart Assessment" (full reset). No single button does two jobs.
  - **Re-take MMSE (`App.tsx` `handleRetake`):** preserves mode + details, clears `mmse`/`mmseScore`/`result`, sets `phase='mmse'` (MMSE intro) and saves synchronously. The Analysis form control is also labeled "Re-take MMSE" (was "Re-take Assessment").
  - **Back matrix (verified):** Assessment Details → Assessment Mode (session preserved, details retained, refresh lands on Mode); MMSE intro → Assessment Details (NEW `← Back` in `MMSEIntroduction` via `onExitToDetails`); MMSE section N → section N-1 (footer/back); MMSE Summary → section 11 (Copying); Analysis → MMSE Summary (NEW `← Back` in the Analysis form via `handleBackToSummary`, which pins the MMSE step to the summary step so the score is preserved).
  - **Regression:** no changes to scoring, hybrid 29+1 AI, Q6/Q11, provider, `/predict`, ML, SHAP, or the session schema. `npm run build` passes; backend suite 85/85 (no backend changes). **Browser verification (CDP, no new deps):** new navigation suite **44/44 PASS** (Tests A–H + patient pending-summary restart regression), session-persistence suite re-ran **41/41 PASS**, patient pending-examiner suite re-ran **40/40 PASS**, and the intro-Back check **6/6 PASS** — no regression.
- **Full assessment-session persistence (refresh-safe):** the WHOLE in-progress assessment survives a browser refresh (`frontend/src/mmse/session.ts`, key `alzheimers_assessment_session_v1`, `SESSION_VERSION = 1`). One authoritative startup restore in `App.tsx` rebuilds the exact phase + MMSE step/phase/state; a 400ms-debounced save effect persists changes (skipped when the session is empty). Restoring an `'assessing'`/`'error'` MMSE phase maps to `'collect'`; Q11 photos are NEVER persisted (`previewData`/`previewName` stripped on save, `photoInfo` cleared on restore, restored as "Choose/Take Photo" while keeping the AI score so the running total stays accurate). The legacy `alzheimers_assessment_details_v1` key is migrated into the session once and removed; corrupt/incompatible sessions are rejected and cleared safely (never crash).
...
- **Assessment Details range validation (UX-fix milestone, committed `7616853` `fix(ux): clarify assessment detail ranges`):** Frontend-only. Made the allowed ranges explicit and enforced them consistently from ONE config source.
  ...
  - **Verified:** `npm run build` passes; backend suite 85/85 (no backend changes). **Browser verification (CDP, `cdp-details.mjs`) 39/39:** empty form disabled; range help shown for all three number fields; age 49/101 invalid w/ error + disabled, 50/100 valid + immediately enabled; education −1/26 invalid (26 w/ error), 0/25 valid, `012`→`12`; ses 0/6 invalid w/ error, 1/5 valid; sex empty disabled / Male enabled; Continue advances to the MMSE intro; refresh restores the intro (persistence); Back retains values; Restart → Mode + key cleared; layer-level session-restore rejects age 49/101, edu −1/26, ses 0/6 and accepts the boundaries (50+25, 100+0) — proving `detailsValid` rejects values the numeric input sanitizer cannot even type. Regression suites re-ran clean: `cdp-persist.mjs` 41/41, `cdp-nav.mjs` 44/44. `/predict` payload (`detailsToPatientInput`), `App.tsx`/`api.ts`/`session.ts` untouched (git diff clean).
- **Q6 naming stimulus asset quality (assets-only milestone, committed `e991eb8` `fix(mmse): improve naming stimulus assets`):** Replaced all 12 Q6 object SVGs under `frontend/public/objects/` (previously thin stroke-only `#e5e7eb` line icons at `viewBox="0 0 200 200"` — abstract/icon-like, hard for a patient to recognize) with clear flat illustrations: `viewBox="0 0 1024 1024"`, bright realistic color fills, consistent dark `#1f2937` outlines, transparent background (renders on the app's near-black card), objects centered with a consistent visual footprint. Filenames/IDs unchanged (`apple ball book chair comb cup fork key pencil scissors umbrella wristwatch`), so `NAMING_OBJECTS` paths, batch slot keys `naming.wristwatch`/`naming.pencil`, expected answers, deterministic scoring, and the Q6 2-point total are untouched. Assets-only: no `.ts`/`.tsx`/backend changes.
  - **Per-object design minima:** wristwatch (leather strap, bezel, white face, ticks, rotated hour/minute/second hands, crown); pencil (pink eraser, silver ferrule, yellow body, sharpened wood tip + graphite); key (round bow with evenodd hole, shoulder, notched shaft, 2 teeth); cup (tapered mug, dark interior, handled outline via dark-underlay/light-overlay); ball (white circle + two crossing red stitched seams — unambiguous vs. apple); book (red cover, dark spine, right-side white page block + page lines); scissors (silver crossing blades, red/blue finger loops via evenodd, pivot screw); comb (amber body + 14 rounded teeth); fork (handle, neck, 4 tines); chair (slatted backrest, wide seat, apron, 4 legs); apple (organic red body with top notch, highlight, brown stem, rotated green leaf); umbrella (scalloped red canopy with Q-curve scallops, 4 ribs, central shaft + finial, dark-underlay/light-overlay J-hook handle — the most scrutinized object).
  - **Verified:** `npm run build` passes; backend suite untouched. **Browser verification (CDP, `cdp-q6.mjs`) 16/16:** examiner mode — Naming section still scored `0 / 2` (max 2 unchanged), exactly 2 object selectors each with the 12 unchanged ids, both stimulus imgs rendered `w-36 h-36 object-contain`, all 12 objects load & decode (naturalWidth/Height > 0, visible non-zero size) in both slots, changing the selector changes the image (apple vs umbrella distinct); patient mode — lands on Naming, no selector shown, 2 stimulus images render & decode, expected answers NOT exposed, "What is this?" prompt present, session stores two distinct random objects (key/chair). Rendered gallery screenshot for visual review: `C:\Users\sumed\AppData\Local\Temp\opencode\q6-gallery.png`.
- **Clarified restart / re-take / back navigation (UX-fix milestone, committed `d6731e1` `fix(ux): clarify restart and back navigation`):** Frontend-only. The three actions are now unambiguously separated and share ONE authoritative full-reset function. Verified in headless Chrome (CDP, no new deps) — new navigation suite **44/44 PASS**; persistence suite re-ran **41/41 PASS**, patient pending-examiner suite **40/40 PASS**, intro-Back check **6/6 PASS**.
  - **Root cause:** the MMSE section header's "Restart" button AND the MMSE Summary's "Restart Assessment" link both called the MMSEAssessment-local `restart` (an MMSE-only reset → MMSE intro), so "Restart Assessment" silently behaved like a re-take; there was also no Back from the MMSE intro or the Analysis form, and no shared full-reset function with an explicit contract.
  - **One authoritative full reset (`App.tsx` `handleRestartAssessment`, renamed from `handleRestart`):** `clearSession()` (removes `alzheimers_assessment_session_v1` + legacy `alzheimers_assessment_details_v1`) → reset `mode`→`DEFAULT_ASSESSMENT_MODE`, `details`→`EMPTY_DETAILS`, `mmse`/`mmseScore`/`result`/`error` → `phase='mode'`. Reused by EVERY "Restart Assessment" control: Assessment Details, MMSE Summary footer, Analysis form, AND the new MMSE section-header button. The empty-session save guard ensures the cleared key is never re-written, so restart → refresh still lands on Assessment Mode (Tests A/H verified).
  - **Re-take MMSE (`App.tsx` `handleRetake`, labeled "Re-take MMSE" everywhere incl. the Analysis form):** preserves mode + details, clears `mmse`/`mmseScore`/`result`, sets `phase='mmse'` (MMSE intro) and saves synchronously. The MMSE header button runs a confirm + local MMSE reset + `onRetake`. Returns to the MMSE intro, never to Assessment Mode (Test B verified; refresh lands on the intro).
  - **Header:** the ambiguous "Restart" is now TWO explicit buttons — "Re-take MMSE" and "Restart Assessment" — no single button does two jobs.
  - **Back = navigation only, never clears the session:** Assessment Details → Assessment Mode (details retained, session `appPhase='mode'`, refresh → Mode — Test C); MMSE intro → Assessment Details (NEW `← Back` in `MMSEIntroduction` via `onExitToDetails`, 6/6 verified); MMSE section → previous section (Test D, responses preserved incl. refresh — Test G); MMSE Summary → Section 11 Copying with responses/AI scores preserved (Test E); Analysis → MMSE Summary via new `handleBackToSummary` (pins MMSE step to the summary step) with the score preserved (Test F).
  - **Regression:** scoring, hybrid 29+1 AI, Q6/Q11, provider, `/predict`, ML, SHAP, and the session schema are untouched. `npm run build` passes; backend suite 85/85 (no backend changes). **Verification:** `cdp-nav.mjs` 44/44 (Tests A–H + patient pending-summary "Restart Assessment" now does a FULL reset, not a re-take — R6/R8); `cdp-persist.mjs` 41/41; `cdp-patient.mjs` 40/40; `cdp-navb.mjs` 6/6 (intro Back).
- **Patient flow with pending examiner sections (UX-fix milestone, committed `7c97059` `fix(ux): allow patient flow with pending examiner sections`):** Frontend-only. Pure Patient mode no longer gets stuck at examiner-dependent sections — Orientation to Place and Q11 Copying become "Pending examiner verification" once the patient's input is complete, navigation (Next + Enter) is enabled, NO score is fabricated, and the final summary refuses to finalize/continue until an examiner resolves them. Verified in headless Chrome (CDP, no new deps) — **40/40 checks PASS**; the previous session-persistence suite re-ran **41/41 PASS (no regression)**.
  - **Root cause:** `sectionResponseCounts().orientationPlace` required BOTH a configured location field AND a response, and pure Patient mode never sets the location → `done` stayed 0 → `isSectionResponseComplete` false → Next disabled forever; `buildBatchItems` also skips unconfigured place items so they can never be AI-scored. The old fix (option A) kept the block and only EXPLAINED it.
  - **Explicit state model (`frontend/src/mmse/batch.ts`):** `SectionStatus = 'incomplete' | 'pending-examiner' | 'complete'` + `getSectionStatus(id, state, mode)`; `canNavigateSection(id, state, mode, phase)`; `sectionResponseCounts(state, mode?)` and `isSectionResponseComplete(id, state, mode?)` are now MODE-AWARE — in Patient mode Orientation to Place counts the 5 responses (no location needed) and Copying counts the patient portion as done; examiner/combined modes keep the strict semantics (location must be configured, Copying must be finalized). Pending-examiner is DERIVED state (no `MMSEState`/`session.ts` schema change), so refresh/persistence is untouched.
  - **Navigation rules:** response incomplete → Next/Enter disabled ("Complete all items to continue"); response complete but not scoreable without the examiner → Next/Enter ENABLED + amber "Pending examiner verification" (in-section box AND nav message, replacing "Examiner required"); scoreable → normal. Verified: Enter with 0 place responses does NOT advance, Next disabled at 4/5, Next enabled at 5/5, Enter advances to Section 3, and the section is stored with responses but NO `aiScore`/`manual` (status `idle`).
  - **Q11 Copying (patient):** shows "Pending examiner verification", Next/Enter enabled (patient not stuck), no photo/analyze offered, and `copying.status` stays `'empty'` with no `aiScore`/`manual` — no fake score. The patient can finish the patient-response portion.
  - **Final summary gating:** with any pending-examiner section the summary shows "Assessment requires examiner verification" + a list ("Orientation to Place — Pending examiner verification", "Copying — Pending examiner verification"); it does NOT show "MMSE Assessment Complete", does NOT offer "Continue to Analysis", and the score is explicitly non-final. The pending card survives a refresh.
  - **Hand off to examiner:** new `handleHandoffToExaminer` (`App.tsx`) switches the session mode to `'examiner'` preserving ALL data (mode, details, responses, step — persisted immediately). A "Hand off to examiner" button appears on the pending summary card and in the Section 2 / Q11 pending boxes (`onHandoffToExaminer` threaded through `MMSEAssessment`, `SectionProps`, `MMSESummary`). After hand-off the normal collect summary returns and the examiner can configure the location, run the AI batch, and take/upload the Q11 photo to finalize.
  - **Examiner + Patient unchanged:** with the location configured, Orientation to Place is evaluated/scored normally, no pending state appears, and the full flow (incl. Q11 photo analysis) finalizes the score (verified end-to-end with a real batch: total 27, "Continue to Analysis" available).
  - **Regression:** no changes to deterministic scoring rules, hybrid 29+1 AI architecture, Q6 2-point scoring, Q11 provider, Q11 image compression, Ollama/Gemma, `/predict`, ML, SHAP, or the session schema. `npm run build` passes; backend suite 85/85 passes (no backend changes). **Browser verification 40/40** (Patient flow incl. Enter + refresh + Q11 + summary gating + hand-off; Examiner + Patient scoring) and the persistence suite **41/41** (matrix A–G + corrupt/legacy/re-take).
- **Full assessment-session persistence (UX-fix milestone, committed `8f00f61` `fix(ux): persist and restore active assessment session`):** Frontend-only. A browser refresh now restores the exact in-progress location (Assessment Mode / Details / MMSE step / Analysis form) with all collected responses and AI results. Verified in headless Chrome (CDP, no new deps) — **41/41 checks PASS** (test matrix A–G + fresh/corrupt/legacy/re-take scenarios).
...
- **Patient flow with pending examiner sections (prior milestone, committed `7c97059`):** Patient mode no longer gets stuck at Orientation to Place or Q11. `SectionStatus` (`batch.ts`) distinguishes incomplete / pending-examiner / complete; Patient mode counts its own input (5 place responses, Copying patient portion), enables Next/Enter, and marks the sections "Pending examiner verification" WITHOUT assigning any score. The final summary shows "Assessment requires examiner verification", lists pending sections, and withholds "Continue to Analysis". "Hand off to examiner" switches the session to examiner mode preserving all data. Examiner + Patient scoring is unchanged (verified with a real batch).
...
- `frontend/src/App.tsx` (`handleRestart` renamed to authoritative `handleRestartAssessment` (full reset → Assessment Mode, `clearSession()` + reset + `phase='mode'`); new `handleBackToSummary` for Analysis → Summary; `MMSEAssessment` wired with `onRetake`/`onRestartAssessment`/`onExitToDetails`; Analysis form: "← Back" + "Re-take MMSE" + "Restart Assessment")
- `frontend/src/components/mmse/MMSEAssessment.tsx` (header now has TWO explicit buttons — "Re-take MMSE" (confirm + local MMSE reset + `onRetake`) and "Restart Assessment" (`onRestartAssessment`); Summary `onRestart` → full reset; new `onRetake`/`onRestartAssessment`/`onExitToDetails` props; intro gets `onBack`)
- `frontend/src/components/mmse/MMSEIntroduction.tsx` (NEW optional `onBack` "← Back" to Assessment Details, session preserved)
- `frontend/src/mmse/batch.ts` (NEW `SectionStatus` type + `getSectionStatus(id, state, mode)` + `canNavigateSection(id, state, mode, phase)`; `sectionResponseCounts(state, mode?)` and `isSectionResponseComplete(id, state, mode?)` now mode-aware — Patient mode counts its own input for Orientation to Place and treats Copying as the patient portion done; examiner semantics unchanged)
- `frontend/src/components/mmse/MMSESummary.tsx` (Patient-mode pending card: "Assessment requires examiner verification" + pending-section list, NO Continue to Analysis, "Hand off to examiner" button; footer "Restart Assessment" now routed to the full reset)
- `frontend/src/components/mmse/sections.tsx` (`SectionProps.onHandoffToExaminer`; Orientation to Place + Copying patient boxes now show "Pending examiner verification" / "Examiner verification required to score" + a "Hand off to examiner" button)
